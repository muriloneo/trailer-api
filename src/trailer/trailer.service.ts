...
export class TrailerService {
  private youtubeURL: string = 'http://youtu.be/watch?v=';
  constructor(private http: HttpService) { }

  async getViaplayIMDB(vp_link: string): Promise<string> {
    try {
      const { data } = await this.http.get(vp_link).toPromise();
      const { id: imdbID } = data._embedded["viaplay:blocks"][0]._embedded["viaplay:product"].content.imdb;
  ...
  async getTrailerMedia(imdbId: string): Promise<string> {
    try {
      ...
      const { key: firstTrailer } = data.results.find((media) => media.type.toLowerCase() === 'trailer');
